...
export default function DisplayQuestion({handleFlip, word}: DisplayAnswerProps) {
 ...
 ">
            ...
            <div className="flex flex-col gap-6 mt-8">
                <p className="text-4xl text-[#2D554D] text-center font-semibold">{word.name}</p>
            </div>

            <button
                className="w-fit text-2xl mt-8 px-10 py-2 border border-green-800 rounded-3xl bg-green-800 text-white hover:cursor-pointer "
                onClick={handleFlip}
            >
                Reveal Answer
            </button>
